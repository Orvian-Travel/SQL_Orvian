...
CREATE OR ALTER VIEW VW_CONFIRMED_RESERVATIONS_SUM_PAYMENTS_BY_DESTINATION AS
    SELECT
        P.DESTINATION                               AS PACKAGE_DESTINATION,
        COUNT(DISTINCT R.ID)                        AS QTD_RESERVAS_CONFIRMADAS,
        COALESCE(SUM(PAY.VALUE_PAID), 0)            AS SOMA_PAGAMENTOS_APROVADOS
    FROM TB_PACKAGES P
    INNER JOIN TB_PACKAGES_DATES PD ON PD.ID_PACKAGE = P.ID
    INNER JOIN TB_RESERVATIONS R ON R.ID_PACKAGES_DATES = PD.ID
    INNER JOIN TB_PAYMENTS PAY ON PAY.ID_RESERVATION = R.ID
    WHERE 
        R.SITUATION = 'confirmada'
        AND PAY.PAYMENT_STATUS = 'aprovado'
    GROUP BY
        P.DESTINATION
GO
