...
SELECT * FROM VW_RESERVATIONS_BY_USER_EMAIL WHERE USER_EMAIL = 'matheus-souza92@jammer.com'
GO
...
-- SELECT PARA EXTRAIR AS INFORMAÇÕES
SELECT * FROM  VW_CONFIRMED_RESERVATIONS_BY_USER_EMAIL_CONFIRMED WHERE USER_EMAIL = 'sonia_dacunha@glaudeimar.com'
GO
...
-- SELECT PARA EXTRAIR AS INFORMAÇÕES
SELECT * FROM  VW_CONFIRMED_RESERVATIONS_BY_USER_EMAIL_CANCELED WHERE USER_EMAIL = 'pedro-darosa95@arysta.com'
GO
...
SELECT
    ...
    P.PRICE                AS PRICE,
    P.TITLE                AS PACKAGE_TITLE,
    CAST(R.RESERVATION_DATE AS DATE)     AS RESERVATION_DATE,
    R.SITUATION            AS SITUATION,
    U.EMAIL                AS USER_EMAIL,  
    (
        SELECT COUNT(*) 
...
-- =========================================================================
-- VIEW: VW_CONFIRMED_RESERVATIONS_SUM_PAYMENTS_BY_DESTINATION
-- OBJETIVO: EXIBIR PARA CADA DESTINO DE PACOTE:
--           - QUANTIDADE DE RESERVAS CONFIRMADAS
--           - SOMA DOS PAGAMENTOS APROVADOS RELACIONADOS AO DESTINO
-- =========================================================================
CREATE OR ALTER VIEW VW_CONFIRMED_RESERVATIONS_SUM_PAYMENTS_BY_DESTINATION AS
SELECT
    P.DESTINATION                         AS package_destination,
    P.TITLE                               AS package_title,
    DATEPART(YEAR, R.RESERVATION_DATE)    AS reservation_year,
    DATEPART(ISO_WEEK, R.RESERVATION_DATE) AS reservation_week,
    COUNT(DISTINCT R.ID)                  AS confirmed_reservations_count,
    COALESCE(SUM(PAY.VALUE_PAID), 0)      AS approved_payments_sum
FROM TB_PACKAGES P
INNER JOIN TB_PACKAGES_DATES PD ON PD.ID_PACKAGE = P.ID
INNER JOIN TB_RESERVATIONS R ON R.ID_PACKAGES_DATES = PD.ID
INNER JOIN TB_PAYMENTS PAY ON PAY.ID_RESERVATION = R.ID AND PAY.PAYMENT_STATUS = 'APROVADO'
WHERE R.SITUATION = 'CONFIRMADA'
GROUP BY
    P.DESTINATION,
    P.TITLE,
    DATEPART(YEAR, R.RESERVATION_DATE),
    DATEPART(ISO_WEEK, R.RESERVATION_DATE)
GO

SELECT * FROM VW_CONFIRMED_RESERVATIONS_SUM_PAYMENTS_BY_DESTINATION
GO

-- =========================================================================
-- VIEW: VW_CANCELLED_RESERVATIONS_BY_DESTINATION
-- OBJETIVO: EXIBIR PARA CADA DESTINO DE PACOTE:
--           - QUANTIDADE DE CANCELAMENTOS
--           - QUANTIDADE TOTAL DE RESERVAS LIGADAS AO PACOTE
-- =========================================================================
CREATE OR ALTER VIEW VW_CANCELLED_RESERVATIONS_BY_DESTINATION AS
    SELECT
        P.DESTINATION                                 AS PACKAGE_DESTINATION,
        COUNT(CASE WHEN R.SITUATION = 'cancelada' THEN 1 END) AS QTD_CANCELAMENTOS,
        COUNT(CASE WHEN R.SITUATION = 'pendente' THEN 1 END) AS QTD_Pendente,
        COUNT(CASE WHEN R.SITUATION = 'confirmada' THEN 1 END) AS QTD_CONFIRMADAS,
        COUNT(R.ID)                                   AS QTD_RESERVAS_TOTAL
    FROM TB_PACKAGES P
    INNER JOIN TB_PACKAGES_DATES PD ON PD.ID_PACKAGE = P.ID
    INNER JOIN TB_RESERVATIONS R ON R.ID_PACKAGES_DATES = PD.ID
    GROUP BY
...
SELECT * FROM VW_CANCELLED_RESERVATIONS_BY_DESTINATION