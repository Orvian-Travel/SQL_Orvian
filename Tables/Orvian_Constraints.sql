...
ALTER TABLE TB_RESERVATIONS
    ...
    CHECK (RESERVATION_DATE >= CONVERT(date, GETUTCDATE()))
GO

ALTER TABLE TB_RESERVATIONS
    ADD CONSTRAINT CHK_CANCEL_DATE_FUTURE
    CHECK (CANCEL_DATE IS NULL OR CANCEL_DATE >= CONVERT(date, GETUTCDATE()))
GO
...
ADD CONSTRAINT CHK_PAYMENTS_METHOD CHECK (
    PAYMENT_METHOD IN ('CREDITO', 'DEBITO', 'BOLETO', 'PIX')
);
GO
...
CHECK (START_DATE <= DATEADD(YEAR, 2, GETUTCDATE()));
GO
