...
ADD CONSTRAINT FK_USER_RESERVATIONS FOREIGN KEY (ID_USER) REFERENCES TB_USERS(ID) ON DELETE CASCADE
GO
...
GO
ALTER TABLE TB_PAYMENTS
ADD CONSTRAINT FK_PAYMENTS_RESERVATION FOREIGN KEY (ID_RESERVATION) REFERENCES TB_RESERVATIONS(ID) ON DELETE CASCADE;
GO


-- Adicionando coluna de FK na tabela de pagamentos
ALTER TABLE TB_PAYMENTS
ADD ID_PROMOTION UNIQUEIDENTIFIER NULL;
GO

-- Adicionando a constraint de chave estrangeira
ALTER TABLE TB_PAYMENTS
ADD CONSTRAINT FK_PAYMENTS_PROMOTION
FOREIGN KEY (ID_PROMOTION) REFERENCES TB_PROMOTIONS(ID);
GO
...
GO
ALTER TABLE TB_TRAVELERS
ADD CONSTRAINT FK_TRAVELERS_RESERVATIONS FOREIGN KEY (ID_RESERVATION) REFERENCES TB_RESERVATIONS(ID) ON DELETE CASCADE;
GO
...
-- TB_PACKAGES
-- Adiciona coluna de chave estrangeira de promoção na tabela TB_PACKAGES
ALTER TABLE TB_PACKAGES
ADD ID_PROMOTION UNIQUEIDENTIFIER NULL;
GO

-- Adiciona constraint de chave estrangeira para promoção
ALTER TABLE TB_PACKAGES
ADD CONSTRAINT FK_PACKAGES_PROMOTION
FOREIGN KEY (ID_PROMOTION) REFERENCES TB_PROMOTIONS(ID);
GO
...
-- Cria índice para a chave estrangeira de promoção na tabela TB_PACKAGES
CREATE INDEX IDX_PACKAGES_ID_PROMOTION
ON TB_PACKAGES(ID_PROMOTION);
GO

-- Cria índice para a chave estrangeira de promoção na tabela TB_PAYMENTS
CREATE INDEX IDX_PAYMENTS_ID_PROMOTION
ON TB_PAYMENTS(ID_PROMOTION);
GO